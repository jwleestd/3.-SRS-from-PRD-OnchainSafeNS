...
- **v0.34 보완: Credential 기반 간소화 이체 UX 및 사기 방지 정책 강화 — (1) 이체 UI "이름 + 금액" 단 2개 입력 간소화 + 체인·자산 자동 결정(Auto-Select) 로직 도입 (2) 사기 주소 검증 시 강제 차단(Hard Block) 정책 — 경고 후 진행 허용 → 이체 절차 원천 중단으로 변경 (3) 이체 완료 후 송수신 양측 통합 알림 통지 의무화 (4) Safe-Name Credential 개념 정의 + 용어집 추가 (5) 미지원 네트워크 UI 레벨 사전 방지 명시 (6) 이체 결과 통지 메시지 필수 항목 정의 (7) TRANSFER_VERIFICATION_LOG 엔터티 통지 필드 확장**
- **v0.35 보완: 일회용 주소(Disposable Address) 기반 온체인 보안 강화 — (1) HD Wallet(BIP-44) 파생 기반 거래 단위 일회용 수신 주소 자동 생성 (2) 일회용 주소 → 영구 주소 자동 포워딩(Meta-Transaction 방식, 수신자 가스비 0원) (3) 일회용 주소 생명주기 관리(생성→활성→사용 완료→만료 24h→폐기) (4) 기존 "이름 + 금액" 이체 UX 투명 통합(송금자 UX 무변경) (5) Disposable Address Service 신규 컴포넌트 + DISPOSABLE_ADDRESS 엔터티 (6) 키 관리 KMS 연동 + 포워딩 가스비 L2 최적화(건당 $0.01 이하) (7) Simulation Mode 일회용 주소 Mock 대응**
...
**(v0.34 추가) 핵심 이체 원칙:** 고객은 사전 등록된 Safe-Name Credential(이름↔온체인 주소 매핑)을 기반으로 **"이름"과 "금액"만 입력**하여 이체를 수행한다. 시스템은 credential을 자동 리졸브하여 사기 주소 여부를 검증하고, 체인·자산을 자동 결정하며, 사기 주소 확인 시 이체를 강제 차단(Hard Block)하고 송금자에게 통지한다. 정상 이체 완료 시 송수신 양측에 이체 결과를 통지한다.
...
| **CON-22** | **(v0.35 신규)** 일회용 주소 자금 포워딩 가스비는 플랫폼이 부담하며, L2(Polygon/Arbitrum) 우선 사용으로 건당 $0.01 이하를 유지해야 한다. Ethereum L1 포워딩 시 건당 $0.50 이하. 월간 총 포워딩 가스비가 $500을 초과하면 운영자 알림을 발동한다 | 가정 | PoC 환경 L2 포워딩 가스비 실측 (1,000건 × 30일) | MVP 착수 전 |
| **CON-23** | **(v0.35 신규)** HD Wallet Seed는 KMS(AWS KMS 또는 HashiCorp Vault)에 저장하며, 파생 개인키는 포워딩 서명 후 메모리에서 즉시 삭제(Zero-Copy Wipe)해야 한다. 시드 접근은 Disposable Address Service만 허용하며, 접근 로그를 감사 로그에 기록한다 | 정책 | KMS 접근 감사 + 메모리 삭제 검증 테스트 | MVP 착수 전 |
| **CON-24** | **(v0.35 신규)** 일회용 주소의 만료 시간(기본 24시간)은 운영기관 시스템 운영 콘솔(OC-5)에서 설정 가능해야 한다. 만료 후 미포워딩 잔금이 있는 경우 자동 포워딩을 시도하며, 3회 실패 시 운영자 에스컬레이션을 발동한다 | 정책 | 만료 후 잔금 포워딩 자동화 테스트 | MVP 개발 중 |
...
| **Safe-Name Credential** | **유저가 Safe-Name 등록 시 생성되는 이름↔온체인 주소 매핑 정보. 이름 기반 이체 시 자동 리졸브·사기 검증·체인 결정의 기반이 되는 사전 등록 자격 증명 (v0.34 신규)** |
| **Chain-Asset Auto-Select** | **이름 + 금액만으로 이체 시, 수신자의 Safe-Name Credential에 등록된 supported_chains/supported_assets를 기반으로 시스템이 최적의 체인·자산을 자동 결정하는 로직. 단일 옵션 시 자동 확정, 다중 옵션 시 선택지 제시(미지원 체인 disabled 처리) (v0.34 신규)** |
| **Hard Block (강제 차단)** | **사기 DB에 등록된 수신자 주소로의 이체를 시스템이 원천 차단하는 정책. 송금자에게 차단 사유를 통지하며, 이체 진행 옵션을 제공하지 않음 (v0.34 신규 — 기존 경고 후 진행 허용 정책 대체)** |
| **Disposable Address (일회용 주소)** | **금융거래 시마다 HD Wallet 파생 경로에서 생성되는 일회성 수신 주소. 단일 거래에만 사용되며, 사용 완료 또는 만료(24시간) 후 자동 비활성화. 입금된 자금은 수신자의 영구 주소로 자동 포워딩됨 (v0.35 신규)** |
| **HD Wallet (Hierarchical Deterministic Wallet)** | **BIP-44 표준 기반 결정적 키 파생 지갑. 단일 시드(Seed)에서 무한 개의 자식 키·주소를 파생할 수 있으며, 일회용 주소 생성의 기반 기술 (v0.35 신규)** |
| **Forwarding (자금 포워딩)** | **일회용 주소에 입금된 자금을 수신자의 영구 지갑 주소로 자동 전송하는 프로세스. Meta-Transaction(가스비 대납) 방식으로 수신자 가스비 0원 (v0.35 신규)** |
| **Forwarding Relayer** | **일회용 주소의 입금을 감지하고 영구 주소로 자금 포워딩 트랜잭션을 제출하는 오프체인 서비스. 가스비를 플랫폼이 대납 (v0.35 신규)** |
...
graph LR
    ...
    subgraph "On-Chain Fraud Shield Platform"
        ...
        UC21["UC21: Credential 기반<br/>간소화 이체 (v0.34)"]
        UC22["UC22: 일회용 주소<br/>자동 생성·포워딩 (v0.35)"]
    end
...
graph TB
    ...
    subgraph "Core Service Layer"
        ...
        SVC16["Chain-Asset Auto-Select<br/>Service (v0.34)"]
        SVC17["Disposable Address<br/>Service (v0.35)"]
    end
...
| **Safe-Name Resolve API** | 내부 REST | human_name 또는 address | 매칭된 주소/이름 + 사기 DB 교차 결과 + **등록 상태·만료일 (v0.31)** + **KYC 검증 등급·Verified 배지·수신 가능 체인·자산 (v0.33)** + **recommended_chain·recommended_asset (v0.34)** | 응답 <= 500ms, 매칭 정확도 100% |
| **Safe-Name Register API** | 내부 REST | human_name, wallet_address, chain, 소유자 서명, **supported_chains, supported_assets (v0.33)** | name_id, registered_at, expires_at, **annual_fee (v0.31)**, **kyc_tier (v0.33)**, **credential_id (v0.34)** | 유저당 5 req/day, Timeout: 5,000ms **(v0.31 변경: 오프체인 등록으로 단축)**, **(v0.33: Tier-1 KYC 필수)**, **(v0.34: credential 생성 확인)** |
...
| **Pre-Transfer Verification API (v0.34 변경)** | 내부 REST | sender_name, recipient_name, **amount (필수)**, asset **(선택)**, chain **(선택)** — 미입력 시 시스템 자동 결정 | recipient_kyc_tier, verified_badge, chain_compatible, asset_compatible, fraud_db_status, verification_summary, **auto_selected_chain, auto_selected_asset, sender_notified, recipient_notified, hard_blocked** | 응답 <= 1,000ms, 호환성 검증 정확도 100%, **사기 주소 강제 차단율 100%** |
...
| **Disposable Address Generate API (v0.35 신규)** | 내부 REST | recipient_name_id, chain_id, asset_symbol, requested_by (sender_name_id) | disposable_address, expires_at, forwarding_target (영구 주소) | Internal Service Token, 이체 건당 1회 자동 호출, Timeout: 500ms |
| **Forwarding Status API (v0.35 신규)** | 내부 REST | disposable_address 또는 log_id | forwarding_status, forwarding_tx_hash, forwarded_at, gas_cost | User Auth Token, 유저당 50 req/day, Timeout: 1,000ms |
...
#### 3.4.4 Safe-Name 기반 송금 플로우 (v0.35 변경: 일회용 주소 자동 생성 + 포워딩 통합)
...
sequenceDiagram
    ...
    participant AutoSelect as Chain-Asset Auto-Select (v0.34)
    participant KYCSvc as KYC Verification Service (v0.33)
    participant CompatSvc as Asset-Chain Compatibility Service (v0.33)
    participant FraudDB as Fraud Address DB
    participant DispAddr as Disposable Address Service (v0.35)
    participant Chain as Blockchain
    participant Relayer as Forwarding Relayer (v0.35)
    participant NotifyGW as Unified Notification GW (v0.31)
    participant Recipient as 수신자 (고객 B)

    Note over Sender,App: v0.34 간소화 입력: "이름" + "금액"만 입력
    Sender->>App: 수신자 이름 입력 (예: "lee.safe") + 송금 금액 입력
    App->>OffChainDB: Credential 리졸브 — 오프체인 레지스트리 조회 (v0.31)
    alt 이름 등록됨 (status: active)
        OffChainDB-->>App: 매칭 주소 (0x…1234) + 만료일 + kyc_tier + supported_chains/assets [<= 100ms]

        rect rgb(240,248,255)
        Note over App,AutoSelect: v0.34 체인·자산 자동 결정 (Chain-Asset Auto-Select)
        App->>AutoSelect: 수신자 supported_chains/assets + 송금 금액 전달
        alt 단일 체인·자산 조합
            AutoSelect-->>App: 자동 확정 (예: Ethereum/USDC) [<= 200ms]
        else 다중 체인·자산 조합
            AutoSelect-->>App: 선택 가능 목록 (미지원 체인 disabled 처리)
            App-->>Sender: 체인·자산 선택 화면 (미지원 항목 비활성)
            Sender->>App: 체인·자산 선택
        else 수신자 지원 목록 미설정
            AutoSelect-->>App: 기본 체인·자산 추천 + 안내
            App-->>Sender: "수신자 수신 가능 체인·자산 미설정" 안내 + 전체 목록
            Sender->>App: 체인·자산 선택
        end
        end
        ...
        App->>CompatSvc: 자산·체인 호환성 검증 (자동 결정 체인/자산 vs 수신자 지원 체인/자산)
        alt 체인·자산 비호환
        ...
        Note over App,FraudDB: v0.34 사기 DB 교차 조회 + 강제 차단 (Hard Block)
        App->>FraudDB: 매칭 주소 사기 DB 교차 조회
        alt 사기 DB 매칭 — v0.34 Hard Block
            FraudDB-->>App: ⛔ 사기 주소 확인 (위험 등급, 사기 유형, 신고 건수)
            App-->>Sender: "⛔ 이체 차단: 수신자 주소가 사기 주소로 확인되었습니다" + 차단 사유 + 사기 이력 요약 + [사기 주소 상세 조회] + [돌아가기] [<= 300ms]
            Note over App,Sender: 이체 진행 옵션 없음 — 강제 차단
            App->>NotifyGW: 차단 통지 발송 (송금자에게만)
            NotifyGW-->>Sender: 선호 채널로 차단 결과 통지
        else 정상 주소
            FraudDB-->>App: ✅ 정상
        end
        end
        ...
        Note over App,DispAddr: v0.35 일회용 수신 주소 자동 생성 (송금자 UX 투명)
        App->>DispAddr: 일회용 주소 생성 요청 (recipient_name_id, chain, asset)
        DispAddr->>DispAddr: HD Wallet 파생 경로에서 신규 주소 생성
        DispAddr-->>App: disposable_address (0x…TEMP) + expires_at (24h 후) [<= 200ms]
        Note over App,Sender: 송금자 화면에는 Safe-Name만 표시, 일회용 주소는 내부 처리
        end

        App->>Chain: 트랜잭션 실행 (수신지: 일회용 주소 0x…TEMP)
        Chain-->>App: TX 완료

        rect rgb(230,240,255)
        Note over Relayer,Chain: v0.35 자금 포워딩 (비동기)
        Relayer->>Chain: 일회용 주소 입금 감지 (이벤트 리스닝)
        Relayer->>Relayer: 포워딩 TX 서명 (KMS에서 파생키 조회 → 서명 → 키 삭제)
        Relayer->>Chain: 일회용 주소 → 영구 주소(0x…1234)로 자금 포워딩 (Meta-TX, 가스비 플랫폼 부담)
        Chain-->>Relayer: 포워딩 TX 완료
        Relayer->>DispAddr: 포워딩 완료 기록 (forwarding_tx_hash, status → 'forwarded')
        DispAddr->>DispAddr: 일회용 주소 상태 → 'used' → 비활성화
        end

        rect rgb(230,255,245)
        Note over App,Recipient: v0.34 송수신 양측 이체 결과 통지
        App->>NotifyGW: 이체 결과 통지 요청 (송금자 + 수신자, 필수 항목 포함)
        par 병렬 통지
            NotifyGW-->>Sender: 선호 채널로 이체 완료 통지 (수신자명, 금액, 자산, 체인, TX 해시, 시각)
            NotifyGW-->>Recipient: 선호 채널로 입금 완료 통지 (송금자명, 금액, 자산, 체인, TX 해시, 시각)
        end
        end
        App-->>Sender: 송금 완료 안내 화면
    else 이름 미등록 또는 만료
...
| REQ-FUNC-015 | **(v0.34 변경)** 송금자가 수신자의 이름(예: "lee.safe")과 **송금 금액만** 입력하고 "송금" 버튼을 탭하면, 시스템은 이름을 주소로 리졸브하고, **수신자의 Safe-Name Credential에 등록된 supported_chains/supported_assets를 기반으로 체인·자산을 자동 결정(Chain-Asset Auto-Select)**한 후, **수신자의 KYC 검증 등급 확인 + 자산·체인 호환성 검증(Asset-Chain Compatibility Gate) + 사기 DB 교차 조회(Pre-Transfer Recipient Verification)**를 수행한다. **수신자 주소가 사기 DB에 등록된 경우 이체를 강제 차단(Hard Block)하고 송금자에게 차단 사유·사기 이력 요약·신고 안내를 통지한다(이체 진행 옵션 없음).** 수신자가 KYC 미검증(Tier-0)인 경우 경고를 표시하고, 체인·자산 비호환 시 트랜잭션을 차단한다. 정상 이체 완료 시 **송수신 양측에 통합 알림 게이트웨이를 통해 이체 결과(송수신자 이름, 금액, 자산, 체인, TX 해시, 시각)를 통지**한다. | Must | Story 4, AC2, CR-V33-A, CR-V33-C, **CR-V34-A** | **Given** 송금자가 수신자 이름과 금액만 입력 **When** 송금 버튼 탭 **Then** 이름→주소 리졸브 + 체인·자산 자동 결정 + KYC 등급 확인 + 체인·자산 호환성 검증 + 사기 DB 교차 조회 + **사기 주소 시 강제 차단 + 송금자 통지** + 정상 시 확인 화면(KYC 등급·Verified 배지 표시) + 최종 승인 후 TX 실행 + **송수신 양측 이체 결과 통지**. 리졸브 <= 500ms, 전체 사전 검증 <= 1,000ms, 매칭 정확도 100%, **체인 비호환 시 TX 차단율 100%, 사기 주소 강제 차단율 100%, 양측 통지 성공률 >= 99.5%** |
| REQ-FUNC-016 | **(v0.34 변경)** 이름 기반 송금 시 리졸브된 주소가 사기 DB에 등록되어 있으면, 시스템은 **이체를 강제 차단(Hard Block)**하고 송금자에게 차단 사유(사기 유형, 위험 등급, 신고 건수), 사기 이력 요약, 사기 주소 신고 안내를 통지해야 한다. **이체 진행 옵션(체크박스 등)을 제공하지 않는다.** | Must | Story 4, AC3, **CR-V34-B** | **Given** 리졸브된 주소가 사기 DB 등록 상태 **When** Pre-Transfer Verification 수행 **Then** 이체 강제 차단 + 차단 사유·사기 이력 요약·신고 안내 표시. **차단 표시 <= 300ms, 강제 차단율 100%, 이체 진행 경로 0개** |
...
| ID | 요구사항 | Priority | Source | Acceptance Criteria |
|---|---|---|---|---|
| **REQ-FUNC-053** | **(v0.34 변경)** 시스템은 Safe-Name 기반 송금 시 트랜잭션 실행 전 **Pre-Transfer Recipient Verification**을 수행해야 한다. 검증 항목: (가) 수신자 KYC 검증 등급(Tier-0~3) 확인 및 표시, (나) 수신자 지원 체인·자산과 **시스템이 자동 결정한** 송금 체인·자산의 호환성 확인, (다) 사기 DB 교차 조회. 수신자가 Tier-0(미검증)인 경우 "수신자가 신원 미검증 상태입니다" 경고와 확인 체크박스를 표시한다. 체인·자산 비호환 시 트랜잭션을 차단하고 "이 수신자는 [자산명]을 [체인명]에서 수신할 수 없습니다. 지원 체인: [목록]" 안내를 표시한다. **사기 DB에 등록된 수신자 주소인 경우 이체를 강제 차단(Hard Block)하고, 차단 사유·사기 이력 요약·신고 안내를 송금자에게 통지한다. 이체 진행 옵션을 제공하지 않는다.** | Must | CR-V33-A, **CR-V34-B** | **Given** 송금자가 Safe-Name으로 수신자 지정 후 송금 시도 **When** Pre-Transfer Verification 실행 **Then** KYC 등급 확인 + 체인·자산 호환성 검증 + 사기 DB 교차 조회 완료. 전체 검증 <= 1,000ms(p95), 체인 비호환 차단율 100%, KYC 미검증 경고 표시율 100%, **사기 주소 강제 차단율 100%, 이체 진행 경로 0개** |
...
#### F4-E. Credential 기반 간소화 이체 UX 및 이체 결과 통지 (v0.34 신규)

| ID | 요구사항 | Priority | Source | Acceptance Criteria |
|---|---|---|---|---|
| **REQ-FUNC-062** | **(v0.34 신규)** 이체 완료 후 시스템은 **통합 알림 게이트웨이를 통해 송금자(고객 A)와 수신자(고객 B) 양측에 이체 결과를 통지**해야 한다. 통지 메시지에는 다음 필수 항목을 포함한다: (가) 송금자 Safe-Name, (나) 수신자 Safe-Name, (다) 송금 금액 및 자산, (라) 송금 체인, (마) TX 해시, (바) 이체 완료 시각. 각 당사자의 선호 알림 채널(Email, KakaoTalk, SMS 등)로 발송하며, 선호 미설정 시 기본 채널(고객: Email)로 폴백한다. **사기 주소 강제 차단(Hard Block) 시에는 송금자에게만 차단 통지를 발송하고, 수신자에게는 발송하지 않는다.** | Must | CR-V34-A | **Given** 이체 완료(TX 확인) **When** 통합 알림 게이트웨이 통지 트리거 **Then** 송수신 양측에 필수 항목 포함 통지 발송. 통지 발송 <= 30초(TX 확인 후), 통지 성공률 >= 99.5%, 필수 항목 누락 0건 |
| **REQ-FUNC-063** | **(v0.34 신규)** 시스템은 이름 기반 이체 시 **체인·자산 자동 결정(Chain-Asset Auto-Select)** 로직을 적용해야 한다. 송금자가 이름과 금액만 입력하면, 수신자의 Safe-Name Credential에 등록된 `supported_chains`/`supported_assets`를 조회하여 다음과 같이 처리한다: (가) **단일 체인·자산 조합만 존재** — 해당 조합을 자동 확정하고 송금자에게 확인 화면만 표시, (나) **다중 체인·자산 조합 존재** — 지원되는 조합만 선택 가능한 목록을 표시하고 **미지원 체인·자산은 disabled(선택 불가) 처리**, (다) **수신자의 지원 목록이 비어 있는 경우** — "수신자가 수신 가능 체인·자산을 설정하지 않았습니다. 직접 확인 후 진행해 주세요" 안내와 함께 플랫폼 지원 체인·자산 전체 목록을 표시. 자동 결정 시 기본 우선순위: 송금자 현재 네트워크 > 네이티브 자산 > 스테이블코인(USDC > USDT). | Must | CR-V34-A | **Given** 송금자가 이름과 금액만 입력 **When** 송금 버튼 탭 **Then** 수신자 credential 기반 체인·자산 자동 결정. 단일 조합 시 자동 확정 <= 200ms, 다중 조합 시 선택 목록 표시 <= 300ms, **미지원 체인 disabled 처리 정확도 100%** |
| **REQ-FUNC-064** | **(v0.34 신규)** 사기 주소 강제 차단(Hard Block) 시 시스템은 송금자에게 다음 정보를 포함한 **차단 통지**를 즉시 표시하고 통합 알림 게이트웨이를 통해 발송해야 한다: (가) 차단 사유("수신자 주소가 사기 주소로 확인되어 이체가 차단되었습니다"), (나) 사기 유형 및 위험 등급, (다) 관련 신고 건수, (라) "[사기 주소 상세 조회]" 링크, (마) "[사기 주소 신고]" 안내. 차단 통지 화면에서 이체 진행 버튼은 제공하지 않으며, "[돌아가기]" 버튼만 표시한다. | Must | CR-V34-B | **Given** Pre-Transfer Verification에서 사기 DB 매칭 확인 **When** Hard Block 발동 **Then** 즉시 차단 화면 표시 + 알림 발송. 차단 화면 표시 <= 300ms, 필수 항목 누락 0건, 이체 진행 경로 0개 |

#### F4-F. 일회용 주소(Disposable Address) 기반 온체인 보안 강화 (v0.35 신규)

| ID | 요구사항 | Priority | Source | Acceptance Criteria |
|---|---|---|---|---|
| **REQ-FUNC-065** | **(v0.35 신규)** 시스템은 Safe-Name 기반 이체 시, 최종 승인 후 트랜잭션 실행 전에 **Disposable Address Service를 호출하여 수신자 전용 일회용 주소를 자동 생성**해야 한다. 일회용 주소는 HD Wallet(BIP-44) 파생 경로에서 생성되며, 수신자의 Safe-Name별 고유한 파생 인덱스를 순차 할당한다. 생성된 일회용 주소로 트랜잭션을 실행하고, **송금자 화면에는 수신자의 Safe-Name만 표시**한다(일회용 주소는 내부 처리로 투명). Simulation Mode 시 Mock HD Wallet에서 테스트넷 주소를 생성한다. | Must | CR-V35-A | **Given** 송금자가 이체를 최종 승인 **When** Disposable Address Service 호출 **Then** 일회용 수신 주소 생성 + 해당 주소로 TX 실행. 주소 생성 <= 200ms, 송금자 화면에 일회용 주소 미노출, 주소 고유성 100% (중복 0건) |
| **REQ-FUNC-066** | **(v0.35 신규)** 일회용 주소에 자금이 입금되면, **Forwarding Relayer가 자동 감지하여 수신자의 영구 지갑 주소로 자금을 포워딩**해야 한다. 포워딩은 Meta-Transaction(가스비 대납) 방식으로 수행하며 수신자 가스비는 0원이다. 포워딩 가스비는 플랫폼이 부담하고 L2 우선 사용으로 건당 $0.01 이하를 목표한다. 포워딩 TX 완료 후 일회용 주소 상태를 `used`로 전환하고 비활성화한다. **포워딩 실패 시 최대 3회 재시도하며, 3회 연속 실패 시 운영자 에스컬레이션(PagerDuty)을 발동하고 수신자에게 지연 안내를 통지한다.** | Must | CR-V35-A | **Given** 일회용 주소에 자금 입금 확인 **When** Forwarding Relayer 감지 **Then** 영구 주소로 포워딩 TX 실행 + 상태 `used` 전환. 포워딩 완료 <= 5분(L2), <= 15분(L1). 포워딩 성공률 >= 99.9%, 가스비 <= $0.01(L2)/$0.50(L1) |
| **REQ-FUNC-067** | **(v0.35 신규)** 일회용 주소는 다음의 **생명주기 상태**를 관리한다: **(가) generated** — 주소 생성 완료, 입금 대기 중, **(나) active** — 입금 트랜잭션 실행됨, 포워딩 대기 중, **(다) used** — 포워딩 완료, 비활성화됨. 재사용 불가, **(라) expired** — 생성 후 24시간 이내 입금 없음. 만료 처리, **(마) expired_with_balance** — 만료 시점에 잔금이 있는 경우. 자동 포워딩 시도 후 `used`로 전환. 각 상태 전환은 DISPOSABLE_ADDRESS 엔터티에 기록하며, `expired_with_balance` 상태에서 포워딩 3회 실패 시 운영자 에스컬레이션을 발동한다. | Must | CR-V35-A | **Given** 일회용 주소 생성 **When** 24시간 경과 + 미입금 **Then** 상태 `expired` 전환. 입금 후 포워딩 완료 시 `used` 전환. 상태 전환 정확도 100%, 만료 처리 지연 <= 1분 |
| **REQ-FUNC-068** | **(v0.35 신규)** 일회용 주소 적용은 **기존 "이름 + 금액" 이체 UX를 변경하지 않는다(투명 적용)**. 송금자에게는 수신자의 Safe-Name과 확인 화면만 표시되며, 일회용 주소는 시스템 내부에서만 참조된다. 수신자에게는 이체 결과 통지 시 "입금이 완료되었습니다" 안내만 발송하며, 일회용 주소·포워딩 과정은 노출하지 않는다. 단, 수신자의 **포워딩 상태 조회 API**를 통해 포워딩 진행 상황을 선택적으로 확인할 수 있다. | Must | CR-V35-B | **Given** 이체 플로우 수행 **When** 일회용 주소 생성·포워딩 **Then** 송금자·수신자 화면에 일회용 주소 미노출. 기존 UX 변경 0건, 포워딩 상태 조회 API 접근 가능 |
| **REQ-FUNC-069** | **(v0.35 신규)** 시스템은 **미사용 만료(expired) 일회용 주소를 일 1회 배치로 정리(Garbage Collection)**해야 한다. 만료 후 30일 경과 주소의 파생 인덱스는 재사용하지 않으며, GC 완료 기록을 감사 로그에 남긴다. `expired_with_balance` 상태의 주소는 잔금 포워딩 완료 전까지 GC 대상에서 제외한다. 운영기관 시스템 운영 콘솔(OC-5)에서 만료 시간(기본 24시간), GC 주기(기본 일 1회)를 설정할 수 있다. | Should | CR-V35-A | **Given** 만료 30일 경과 일회용 주소 존재 **When** 일일 GC 배치 실행 **Then** 대상 주소 정리 완료 + 감사 로그 기록. GC 실행 <= 5분(1,000건 기준), 잔금 있는 주소 GC 제외 정확도 100% |
...
#### 4.2.11 v0.34 추가 NFR (Credential 기반 간소화 이체)

| ID | 요구사항 | 기준 | 측정 경로 | Source |
|---|---|---|---|---|
| **REQ-NF-048** | **(v0.34 신규)** Chain-Asset Auto-Select 체인·자산 자동 결정 응답 시간 | p95 <= 200ms (단일 조합), p95 <= 300ms (다중 조합 목록 생성) | Datadog APM: `autoselect.latency_p95` | CR-V34-A |
| **REQ-NF-049** | **(v0.34 신규)** 이체 결과 양측 통지 발송 시간 (TX 확인 후) | p95 <= 30초 (송수신 양측 동시) | Datadog: `transfer.notify.latency_p95` | CR-V34-A |
| **REQ-NF-050** | **(v0.34 신규)** 이체 결과 양측 통지 성공률 | >= 99.5% (1차 시도, 실패 시 1회 재시도) | Datadog: `transfer.notify.success_rate` | CR-V34-A |
| **REQ-NF-051** | **(v0.34 신규)** 사기 주소 강제 차단(Hard Block) 차단율 | 100% (사기 DB 등록 주소 이체 통과 0건) | Datadog Custom Metric: `fraud.hard_block.pass_rate` | CR-V34-B |
| **REQ-NF-052** | **(v0.34 신규)** 미지원 체인 UI disabled 처리 정확도 | 100% (미지원 체인 선택 가능 0건) | E2E 테스트 + Datadog | CR-V34-A |

#### 4.2.12 v0.35 추가 NFR (일회용 주소 보안 강화)

| ID | 요구사항 | 기준 | 측정 경로 | Source |
|---|---|---|---|---|
| **REQ-NF-053** | **(v0.35 신규)** 일회용 주소 생성 응답 시간 | p95 <= 200ms (HD Wallet 파생 + DB 기록) | Datadog APM: `disposable.generate.latency_p95` | CR-V35-A |
| **REQ-NF-054** | **(v0.35 신규)** 자금 포워딩 완료 시간 (입금 확인 후) | p95 <= 5분(L2), p95 <= 15분(L1) | Datadog: `forwarding.complete.latency_p95` | CR-V35-A |
| **REQ-NF-055** | **(v0.35 신규)** 자금 포워딩 성공률 | >= 99.9% (1차 시도 + 최대 3회 재시도) | Datadog: `forwarding.success_rate` | CR-V35-A |
| **REQ-NF-056** | **(v0.35 신규)** 포워딩 가스비 (L2 기준) | 건당 <= $0.01 (L2), <= $0.50 (L1) | 온체인 TX 비용 모니터링 | CR-V35-A, CON-22 |
| **REQ-NF-057** | **(v0.35 신규)** 일회용 주소 고유성 (중복 방지) | 중복 생성 0건 | DB 유니크 제약 + 주 1회 전수 스캔 | CR-V35-A |
| **REQ-NF-058** | **(v0.35 신규)** HD Wallet Seed KMS 접근 감사 로그 기록률 | 100% (모든 접근 기록) | KMS Audit Log + CloudTrail | CR-V35-A, CON-23 |
| **REQ-NF-059** | **(v0.35 신규)** 파생 개인키 메모리 삭제(Zero-Copy Wipe) 검증 | 서명 후 메모리 잔존 0건 | 보안 감사 + 메모리 덤프 테스트 | CR-V35-A, CON-23 |
...
| **Story 4-E (Credential Transfer UX, v0.34)** | **REQ-FUNC-062** | **TC-FUNC-062** | **Must** |
| **Story 4-E (Credential Transfer UX, v0.34)** | **REQ-FUNC-063** | **TC-FUNC-063** | **Must** |
| **Story 4-E (Credential Transfer UX, v0.34)** | **REQ-FUNC-064** | **TC-FUNC-064** | **Must** |
| **Story 4-F (Disposable Address, v0.35)** | **REQ-FUNC-065** | **TC-FUNC-065** | **Must** |
| **Story 4-F (Disposable Address, v0.35)** | **REQ-FUNC-066** | **TC-FUNC-066** | **Must** |
| **Story 4-F (Disposable Address, v0.35)** | **REQ-FUNC-067** | **TC-FUNC-067** | **Must** |
| **Story 4-F (Disposable Address, v0.35)** | **REQ-FUNC-068** | **TC-FUNC-068** | **Must** |
| **Story 4-F (Disposable Address, v0.35)** | **REQ-FUNC-069** | **TC-FUNC-069** | **Should** |
...
| **v0.34 NFR (Credential Transfer UX)** | **REQ-NF-048 ~ REQ-NF-052** | **TC-NF-048 ~ TC-NF-052** | **Must** |
| **v0.35 NFR (Disposable Address)** | **REQ-NF-053 ~ REQ-NF-059** | **TC-NF-053 ~ TC-NF-059** | **Must** |
...
| **A19** | **`/api/v1/transfer/verify`** | **POST** | **Pre-Transfer Recipient Verification — 송금 전 수신자 종합 검증 + 체인·자산 자동 결정 (v0.34 변경: chain/asset 선택 입력, 미입력 시 Auto-Select)** | **User Auth Token** | **유저당 100 req/day** | **1,000ms** |
...
| **A22** | **`/api/v1/transfer/notify`** | **POST** | **이체 결과 송수신 양측 통지 발송 (v0.34 신규) — 통합 알림 게이트웨이 연동** | **Internal Service Token** | **서비스당 10,000 req/min** | **30,000ms** |
| **A23** | **`/api/v1/disposable/generate`** | **POST** | **일회용 수신 주소 생성 — HD Wallet 파생 (v0.35 신규). 이체 최종 승인 후 시스템 내부 자동 호출** | **Internal Service Token** | **이체 건당 1회** | **500ms** |
| **A24** | **`/api/v1/disposable/forwarding-status`** | **GET** | **일회용 주소 포워딩 상태 조회 (v0.35 신규)** | **User Auth Token** | **유저당 50 req/day** | **1,000ms** |
| **A25** | **`/api/v1/admin/disposable/config`** | **GET/PUT** | **일회용 주소 만료 시간·GC 주기 설정 (v0.35 신규)** | **Admin MFA Token** | **10 req/day** | **1,000ms** |
...
#### DISPOSABLE_ADDRESS (v0.35 신규)

| 필드 | 타입 | 제약 | 설명 |
|---|---|---|---|
| **disposable_id** | **string** | **PK** | **일회용 주소 레코드 ID** |
...
| **disposable_address** | **string** | **UNIQUE, NOT NULL, INDEXED** | **생성된 일회용 주소** |
| **permanent_address** | **string** | **NOT NULL** | **수신자 영구 지갑 주소 (포워딩 대상)** |
| **derivation_path** | **string** | **NOT NULL** | **HD Wallet BIP-44 파생 경로 (예: m/44'/60'/0'/0/1234)** |
| **derivation_index** | **int** | **NOT NULL** | **파생 인덱스 (순차 할당, 재사용 불가)** |
...
| **hard_blocked** | **boolean** | **NOT NULL, DEFAULT false (v0.34)** | **사기 주소 강제 차단 여부** |
| **auto_selected_chain** | **string** | **NULLABLE (v0.34)** | **자동 결정된 체인 (Auto-Select 결과)** |
| **auto_selected_asset** | **string** | **NULLABLE (v0.34)** | **자동 결정된 자산 (Auto-Select 결과)** |
| **sender_notified_at** | **datetime** | **NULLABLE (v0.34)** | **송금자 이체 결과 통지 일시** |
| **recipient_notified_at** | **datetime** | **NULLABLE (v0.34)** | **수신자 이체 결과 통지 일시** |
| **disposable_address_id** | **string** | **FK → DISPOSABLE_ADDRESS, NULLABLE (v0.35)** | **사용된 일회용 주소 레코드 ID** |
...
| **TRANSFER_VERIFICATION_LOG → NOTIFICATION_PREFERENCE (v0.34)** | **이체 검증 후 양측 통지 시 알림 선호 참조** |
| **SAFE_NAME ∥--o{ DISPOSABLE_ADDRESS (v0.35)** | **Safe-Name 수신자에 대해 다수의 일회용 주소가 생성됨** |
| **TRANSFER_VERIFICATION_LOG ∥--o∥ DISPOSABLE_ADDRESS (v0.35)** | **이체 검증 로그가 사용된 일회용 주소를 참조** |
...
| **H15** | **(v0.34 신규)** "이름 + 금액" 간소화 UI가 사용자 이체 완료율 및 만족도를 향상 | **A/B Test (n=500명): 간소화 UI(이름+금액) vs 기존 UI(이름+금액+체인·자산 선택) 비교** | **이체 완료율, 이체 소요 시간, 사용자 만족도, 체인 선택 오류율** | **이체 완료율 30% 향상, 소요 시간 50% 단축, 만족도 >= 4.5/5, 체인 오류율 0%** |
| **H16** | **(v0.34 신규)** 사기 주소 강제 차단(Hard Block) 정책이 사기 피해를 추가 감소시킴 | **코호트 분석 (n=1,000건): Hard Block 적용 vs 기존 경고+체크박스 비교** | **사기 피해 발생률, 차단 후 신고 전환율, 사용자 만족도** | **사기 피해 95% 감소, 차단 후 신고 전환율 >= 30%, 만족도 >= 4.0/5** |
| **H17** | **(v0.34 신규)** 송수신 양측 이체 결과 통지가 사용자 신뢰도 및 재이용률을 향상 | **A/B Test (n=300명): 양측 통지 vs 송금자만 통지 비교** | **사용자 신뢰도, D30 재이용률, 통지 확인율** | **신뢰도 20% 향상, 재이용률 15% 향상, 통지 확인율 >= 85%** |
| **H18** | **(v0.35 신규)** 일회용 주소가 수신자 주소 추적·프로파일링을 방지 | **보안 분석 (n=1,000건): 동일 수신자 다건 이체 후 온체인 주소 연관 분석** | **주소 연관성 탐지율, 프로파일링 가능 거래 비율** | **연관성 탐지율 <= 5%, 프로파일링 가능 거래 0%** |
| **H19** | **(v0.35 신규)** 일회용 주소 + 자금 포워딩이 기존 이체 UX를 저해하지 않음 | **A/B Test (n=300명): 일회용 주소 적용 vs 미적용 이체 완료율·만족도 비교** | **이체 완료율, 이체 소요 시간(포워딩 포함), 사용자 만족도** | **이체 완료율 변동 <= 2%, 소요 시간 증가 <= 10%, 만족도 >= 4.0/5** |
| **H20** | **(v0.35 신규)** 자금 포워딩 L2 최적화로 가스비 $0.01 이하 유지 | **파일럿 (n=500건): L2(Polygon, Arbitrum) 포워딩 가스비 실측** | **건당 포워딩 가스비, 월간 총 가스비, 포워딩 성공률** | **건당 <= $0.01(L2), 월간 <= $500, 성공률 >= 99.9%** |
