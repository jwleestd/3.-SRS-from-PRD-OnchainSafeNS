...
- v0.6 보완: 품질 리뷰 14건 반영 — GWT AC 전량 반영, 번들 해체, 다이어그램 3종 추가, NFR 모니터링·위반 조치 추가, Traceability 확장
- **v0.7 보완: MVP 기술 스택 정합성 + 구현 커버리지 강화 — (1) C-TEC-010 환경 분기 전략 재정의 (2) Component Diagram 로컬 SQLite/배포 Supabase 이중 환경 반영 (3) Class Diagram → Module Architecture Diagram 전환 (Route Handler→Service→Prisma 계층 명시) (4) Server Actions vs Route Handlers 사용 가이드 신규 (5) Middleware 상세 명세 신규 (6) shadcn/ui 컴포넌트 매핑 신규 (7) 통합 에러 응답 포맷 명세 신규 (8) 환경변수 목록 신규 (9) 시드 데이터 분포 상세 (10) 프로젝트 디렉토리 구조 보강**
...
**v0.7 핵심 변경:** (1) MVP 기술 스택(C-TEC-001~007) 정합성 재검증 — 환경 분기 전략 재정의 (2) Component Diagram에 로컬 SQLite/배포 Supabase 이중 환경 반영 (3) Class Diagram → Module Architecture Diagram 전환 (4) Server Actions vs Route Handlers 사용 가이드 추가 (5) Phase-0 프론트엔드 shadcn/ui 컴포넌트 매핑 추가 (6) Middleware 상세 명세 추가 (7) 통합 에러 응답 포맷 표준화 (8) 환경변수 목록 추가 (9) 시드 데이터 분포 상세화 (10) 프로젝트 디렉토리 구조 보강
...
| C-TEC-003 | Prisma + SQLite(로컬) / Supabase PostgreSQL(배포) — 환경별 분기 | Phase-0~ | Phase-0: Free(500MB) |
...
| C-TEC-010 | 환경별 DB 분기 전략: `DATABASE_URL` 환경변수로 로컬(SQLite) / 배포(Supabase PostgreSQL) 자동 분기. Prisma의 `datasource` 설정으로 별도 마이그레이션 없이 양쪽 호환 | Phase-0~ | Prisma 호환 규칙(§6.2) 준수 |
...
| Prisma | TypeScript ORM. SQLite/PostgreSQL 환경 분기 |
...
| REF-10 | Tech Stack 환경 분기 전략 | C-TEC-010 (SRS §1.4, §6.8) |
...
> v0.7 변경: C-TEC-003에 따라 로컬(SQLite) / 배포(Supabase PostgreSQL) 이중 환경을 명시. 서비스 모듈이 Route Handler에서 호출되는 계층 관계를 반영.
...
graph TB
    subgraph "Next.js App (Phase-0)"
        ...
        PRISMA["lib/prisma.ts (PrismaClient)"]
    end

    subgraph "Database (C-TEC-003 환경 분기)"
        DB_LOCAL["📁 SQLite (로컬 개발)<br/>file:./dev.db"]
        DB_DEPLOY["🐘 Supabase PostgreSQL (배포)<br/>DATABASE_URL 환경변수"]
    end
    ...
    RH3 & RH4 & RH7 --> SVC2
    RH6 --> SVC3
    SVC1 & SVC2 & SVC3 --> PRISMA
    PRISMA -.->|"로컬"| DB_LOCAL
    PRISMA -.->|"배포"| DB_DEPLOY
    SVC1 & SVC2 & SVC3 --> EMAIL
```

### 3.5 Phase-0 Module Architecture Diagram (v0.7 변경)

> v0.7 변경: v0.6의 Class Diagram을 Module Architecture Diagram으로 전환. C-TEC-002에 따라 Route Handler → Service Module → Prisma 3계층 구조를 명시하고, 각 모듈이 Next.js App Router 내부에서 어떻게 호출되는지를 표현한다.
...
classDiagram
    ...
    class FraudService {
        <<lib/services/fraud.ts>>
        +lookupAddress(address, chain) FraudResult
        +submitReport(reportData) FraudReport
        +getPendingReports() FraudReport[]
        +processApproval(reportId, action, notes) void
        -crossCheckFraudDB(address) FraudAddress?
        -notifyReporter(report, action) void
    }

    class SafeNameService {
        <<lib/services/safe-name.ts>>
        +registerName(name, address, chain, userId) SafeName
        +resolve(humanName) ResolveResult
        +executeDemo(recipientName, amount, senderId) TransferDemo
    ...
    class AuthService {
        ...
        +verifyCode(email, code) User
        +checkEmailVerified(userId) boolean
        -generateCode() string
        -isCodeExpired(expiresAt) boolean
    }
    ...
    class EmailService {
        ...
        -callResendAPI(to, subject, body) SendResult
    }
    ...
    class Middleware {
        <<middleware.ts>>
        +matcher string[]
        +middleware(req) NextResponse
        -isAdminAuthenticated(req) boolean
    }

    RouteHandlers --> FraudService : imports & calls
    RouteHandlers --> SafeNameService : imports & calls
    RouteHandlers --> AuthService : imports & calls
    FraudService --> PrismaClient : DB CRUD
    FraudService --> EmailService : sends notifications
    SafeNameService --> PrismaClient : DB CRUD
    SafeNameService --> FraudService : crossCheck on resolve
    SafeNameService --> EmailService : sends notifications
    AuthService --> PrismaClient : DB CRUD
    AuthService --> EmailService : sends code
    Middleware --> RouteHandlers : guards /admin/* routes
```
...
| Route Handler | `app/api/v1/**/route.ts` | HTTP 요청 파싱, 응답 포맷팅, 에러 핸들링 | Service Module |
| Service Module | `lib/services/*.ts` | 비즈니스 로직, 유효성 검증, 트랜잭션 | PrismaClient, EmailService |
| Email Service | `lib/email.ts` | Resend API 호출 | Resend SDK |
| Prisma Client | `lib/prisma.ts` | DB 접근 (싱글턴) | DATABASE_URL 환경변수 |
...
|---|---|---|---|
| **Route Handler** (`route.ts`) | 외부에서 호출 가능한 REST API | ✅ 전 API 적용 | Rate Limit, CORS, 에러 포맷 통일이 필요한 엔드포인트 |
| **Server Action** (`"use server"`) | 페이지 내부 폼 제출 | ⚠ Phase-0 미사용 (Phase-1+ 검토) | Phase-0은 API 중심 아키텍처 유지하여 단순성 확보 |
...
| REQ-FUNC-025 | Agent 대시보드 | Should | — |
...
#### 6.1.1 통합 에러 응답 포맷 (v0.7 신규)

> 모든 Route Handler는 아래 JSON 포맷을 준수한다. 성공/실패 여부와 무관하게 동일 구조를 반환하여 프론트엔드 파싱을 단순화한다.
...
> v0.7 변경: 시드 데이터 분포를 상세화하여 구현자가 `seed.ts` 작성 시 임의 판단 없이 구현할 수 있도록 한다.
...
| | solana | 10건 |
| | arbitrum | 5건 |
| **risk_level** | critical | 20건 |
| | high | 30건 |
| | medium | 30건 |
| | low | 20건 |
| **source_type** | 전량 'seed' | 100건 |
| **status** | 전량 'verified' | 100건 |
| **report_count** | 1~10 (랜덤 분포) | 100건 |
| **address 형식** | 0x + 40 hex chars (EVM) | 85건 |
| | Base58 (Solana) | 10건 |
| | 기타 | 5건 |

#### 6.3.3 SAFE_NAME 분포 (20건)
...
| **status** | active | 17건 |
| | expired (테스트용) | 3건 |
| **human_name 예시** | alice.safe, bob.safe, kim.safe, test-user.safe 등 | — |
| **특수 케이스** | 사기 DB 매칭 주소(FRAUD_ADDRESS와 교차) | 3건 |
| | 정상 주소(사기 DB 미등록) | 17건 |

**사기 DB 매칭 시드:** SAFE_NAME 20건 중 3건은 wallet_address가 FRAUD_ADDRESS의 address와 일치하도록 설정하여, 데모 이체 시 "차단" 시나리오를 테스트할 수 있도록 한다. 예: `evil.safe` → `0xBAD...` (FRAUD_ADDRESS risk_level=critical).

#### 6.3.4 USER 분포 (5건)

| # | email | email_verified | 용도 |
|---|---|---|---|
| 1 | user1@test.com | true | 일반 유저 (조회·신고·등록·이체 테스트) |
| 2 | user2@test.com | true | 일반 유저 (중복 신고 테스트) |
| 3 | user3@test.com | true | 일반 유저 |
| 4 | unverified@test.com | false | 미인증 유저 (401 에러 테스트) |
| 5 | restricted@test.com | true | 신고 제한 유저 (report_restriction_until = 미래 날짜, 429 에러 테스트) |

#### 6.3.5 OPERATOR 분포 (1건)

| # | name | email | role | 용도 |
|---|---|---|---|---|
| 1 | Admin | admin@fraud-shield.com | admin | 승인 대시보드 접근 |
...
> v0.7 변경: 공유 유틸리티, 타입 정의, 컴포넌트 디렉토리를 추가하여 구현 시 파일 위치 혼선을 방지한다.
...
│   ├── api/v1/
│   │   ├── fraud/
│   │   │   ├── lookup/route.ts        ← GET: 사기 주소 조회
│   │   │   └── report/route.ts        ← POST: 사기 신고 접수
│   │   ├── resolve/route.ts           ← GET: Safe-Name 리졸브
│   │   ├── safename/
│   │   │   └── register/route.ts      ← POST: Safe-Name 등록
│   │   ├── transfer/
│   │   │   └── demo/route.ts          ← POST: 데모 이체 시뮬레이션
│   │   ├── admin/
│   │   │   └── approve/route.ts       ← POST: 신고 승인/거부
│   │   └── auth/
│   │       └── verify-email/route.ts  ← POST: 이메일 인증 코드 확인
│   ├── layout.tsx                     ← RootLayout: 폰트, Tailwind, 네비게이션
│   ├── page.tsx                       ← 랜딩/리다이렉트
│   └── globals.css                    ← Tailwind @tailwind 지시자
├── components/
...
│   │   └── separator.tsx
│   └── shared/                        ← 프로젝트 공유 컴포넌트
│       ├── fraud-result-card.tsx       ← 사기 조회 결과 카드
│       ├── safe-name-badge.tsx         ← Safe-Name 상태 배지
│       └── email-verify-form.tsx       ← 이메일 인증 폼 (재사용)
├── lib/
│   ├── services/
│   │   ├── fraud.ts                   ← FraudService 비즈니스 로직
│   │   ├── safe-name.ts               ← SafeNameService + executeDemo()
│   │   └── auth.ts                    ← AuthService 인증 로직
│   ├── email.ts                       ← EmailService (Resend API)
│   ├── prisma.ts                      ← PrismaClient 싱글턴
│   ├── api-response.ts                ← successResponse(), errorResponse(), withErrorHandler()
│   ├── validators.ts                  ← 입력 유효성 검증 (주소 형식, 이름 규칙 등)
│   └── constants.ts                   ← 예약어 목록, Rate Limit 설정, 에러 코드 등
├── types/
│   └── index.ts                       ← FraudResult, ResolveResult, TransferDemo 등 타입 정의
├── prisma/
│   ├── schema.prisma                  ← 6 모델 (USER, SAFE_NAME, FRAUD_ADDRESS, FRAUD_REPORT, OPERATOR, TRANSFER_DEMO)
│   └── seed.ts                        ← §6.3 시드 데이터 (100+20+5+1건)
├── middleware.ts                      ← §3.7 Admin 인증 가드
├── .env.local                         ← §6.8 환경변수 (로컬)
├── .env.example                       ← §6.8 환경변수 템플릿
├── package.json
├── tailwind.config.ts
├── tsconfig.json
├── next.config.ts
...
### 6.8 Phase-0 환경변수 목록 (v0.7 신규)

> 구현자가 `.env.local` / `.env.example` 작성 시 누락을 방지하기 위한 환경변수 전량 목록.

#### 6.8.1 Phase-0 필수 환경변수

| 변수명 | 용도 | 예시 값 (로컬) | 배포 설정 위치 |
|---|---|---|---|
| `DATABASE_URL` | Prisma DB 연결 | `file:./dev.db` (SQLite) | Vercel → Environment Variables: `postgresql://...` (Supabase) |
| `DIRECT_URL` | Prisma 직접 연결 (Supabase Pooler 우회) | (로컬에선 미사용) | Vercel: `postgresql://...?pgbouncer=true` |
| `RESEND_API_KEY` | Resend Email API 키 | `re_test_xxxxx` | Vercel: `re_live_xxxxx` |
| `ADMIN_PASSWORD` | Admin 인증 비밀번호 | `dev-admin-pw-2026` | Vercel: 강력한 비밀번호 (≥16자, 특수문자 포함) |
...
#### 6.8.2 Phase-1+ 추가 환경변수 (참고)

| 변수명 | 용도 | Phase |
|---|---|---|
| `SLACK_WEBHOOK_URL` | Slack 알림 | 1 |
| `PAGERDUTY_API_KEY` | SLA 에스컬레이션 | 1 |
| `ETHERSCAN_API_KEY` | 주소 레이블 수집 | 1 |
| `ALCHEMY_API_KEY` | RPC 연결 | 2 |
| `KYC_PROVIDER_API_KEY` | KYC 검증 | 2 |
| `CHAINALYSIS_API_KEY` | 주소 위험 등급 | 2 |
| `GEMINI_API_KEY` | LLM 추론 (C-TEC-006) | 3 |
| `LLM_MODEL` | LLM 모델 지정 (C-TEC-006: 환경변수로 교체) | 3 |

#### 6.8.3 Prisma schema.prisma datasource 설정

> C-TEC-003 + C-TEC-010 준수: 로컬(SQLite)과 배포(PostgreSQL) 환경을 `DATABASE_URL` 환경변수로 자동 분기한다.

```prisma
datasource db {
  provider  = "postgresql"  // 배포 기준
...
// → Prisma는 provider를 무시하고 URL 스킴으로 SQLite 자동 감지
```
...
| # | 항목 | 설명 |
|---|---|---|
| 1 | SQLite/PostgreSQL 호환 | §6.2 Prisma 호환 규칙(JSON→String+parse, Enum→String 등) 반드시 준수 |
| 2 | 환경변수 노출 금지 | `NEXT_PUBLIC_` 접두사가 없는 변수는 클라이언트에 노출되지 않음. ADMIN_PASSWORD, API Key 류는 절대 `NEXT_PUBLIC_` 사용 금지 |
