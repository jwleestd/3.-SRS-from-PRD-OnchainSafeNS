...
- v0.3.1 보완: CR-1 외부 소스 정책 변경 우회 조치, CR-2 사기 주소 오등록 대응 전략, CR-3 네이밍 서비스 연계·가스비 완화·금융공동망/ISO 20022 지원
- **v0.31 보완: (1) 이종 온체인 네이밍 서비스 미들웨어(NRM) 전략 (2) Safe-Name 독자 오프체인 레지스트리·DNS식 비용 모델 (3) 통합 알림 게이트웨이(Slack·Email·KakaoTalk 등) (4) Stakeholders 3계층(고객·이용기관·운영기관) 분리 및 Client Applications 재배치 (5) 금융결제원 OPEN API 기반 예금주·법인명 조회로 변경 (6) MVP 전 컴포넌트 시뮬레이션 모드 아키텍처**
...
| IS-9 | **금융결제원 OPEN API 기반 예금주·법인명 조회 게이트웨이 + ISO 20022 메시지 게이트웨이 (v0.31 변경)** |
| **IS-10** | **통합 알림 게이트웨이 (Slack, Email, KakaoTalk, SMS 등 멀티채널 Webhook) (v0.31 신규)** |
| **IS-11** | **운영기관용 Admin Console — 플랫폼 운영·모니터링·컴플라이언스 대시보드 (v0.31 신규)** |
...
| **OS-7** | **금융결제원 금융공동망 전문(電文) 직접 인터페이스 (v0.31)** | **OPEN API로 대체, 전문 인터페이스는 향후 검토** |
...
| **CON-18** | **(v0.31 신규)** 금융결제원 OPEN API 이용을 위한 핀테크 서비스 등록 및 이용기관 승인이 MVP 출시 전 완료되어야 한다 | 의존성 | 금융결제원 핀테크 서비스 등록 신청 + 이용기관 협약 체결 | MVP 기획 중 |
...
| ISO 20022 | 금융 메시지 국제 표준 — XML/JSON 기반 금융 메시징 규격 (pacs, pain 등) |
...
| **금융결제원 OPEN API** | **금융결제원이 제공하는 오픈뱅킹 REST API — 예금주 조회, 법인명 조회 등 (v0.31 변경: 기존 전문(電文) 인터페이스에서 변경)** |
...
| REF-07 | ISO 20022 Financial Messaging Standard (pacs.008/pacs.002) | 외부 국제 표준 |
| **REF-08** | **금융결제원 OPEN API(오픈뱅킹) 기술 규격 — 예금주 조회·법인명 조회 API (v0.31 변경: 금융공동망 전문 규격 → OPEN API 규격)** | **금융결제원 오픈플랫폼 (open.kftc.or.kr)** |
...
| TradFi IT팀장 | C3 최수영 — STO 증권사 인프라팀 | On-Premise ZK 모듈 도입, 망분리 규제 준수, 금융결제원 OPEN API 연계, 고객 앞 서비스 운영 | 100% 망분리 가능 On-Premise 모듈, 예금주·법인명 조회 연동, ISO 20022 호환 |
...
| **금융결제원 OPEN API (v0.31 변경)** | **외부 REST (오픈뱅킹)** | **예금주 조회 API(계좌번호 → 예금주명), 법인명 조회 API — Safe-Name ↔ 실명/법인명 KYC 연계. MVP 시뮬레이션 시 Stub 응답** | **핀테크 서비스 등록 필수, API 호출 건당 과금, 일일 호출 한도 존재** |
...
| 금융결제원 OPEN API·ISO 20022 게이트웨이 (API) | C3 (TradFi IT팀장) | Safe-Name 리졸브 결과 기반 예금주·법인명 조회, ISO 20022 메시지 생성·반환 |

#### 3.2.3 Tier-3 운영기관용 Client Applications (v0.31 신규)

| 클라이언트 | 대상 사용자 | 기능 |
|---|---|---|
| **Admin Console (Web)** | O1 (플랫폼 운영자) | 시스템 전체 모니터링 대시보드, SLA 현황, 비용 추적, 알림 채널 관리, Simulation/Production 모드 전환, 이용기관·고객 관리, NRM 어댑터 관리 |
| **컴플라이언스 대시보드 (Web)** | O2 (컴플라이언스 담당) | 이의 신청 심사 대기열, 오등록 판정 워크플로우, Safe-Name 분쟁 관리(UDRP), 금융 규제 준수 감사 로그 |
| **Data QA 대시보드 (Web)** | O3 (Data QA팀) | 사기 DB 커버리지·품질 모니터링, 소스별 커버리지 비교 리포트, 데이터 정합성 검증 결과, 외부 소스 상태 모니터링 |
...
graph LR
    ...
    subgraph "On-Chain Fraud Shield Platform"
        ...
        UC14["UC14: 금융결제원 OPEN API<br/>예금주·법인명 조회 (v0.31)"]
        UC15["UC15: 플랫폼 운영<br/>Admin Console (v0.31)"]
        UC16["UC16: 이의 심사·<br/>오등록 판정 (v0.31)"]
        UC17["UC17: Data QA·<br/>커버리지 모니터링 (v0.31)"]
...
graph TB
    subgraph "Client Layer"
        ...
        subgraph "Tier-2: 이용기관용"
            ...
            CL7["금융결제원 OPEN API<br/>Gateway Client (v0.31)"]
        end
        subgraph "Tier-3: 운영기관용 (v0.31)"
            CL8["Admin Console (Web)"]
            CL9["컴플라이언스<br/>Dashboard (Web)"]
            CL10["Data QA<br/>Dashboard (Web)"]
        end
    end
    ...
    subgraph "Core Service Layer"
        ...
        SVC10["Financial Gateway<br/>Service (OPEN API, v0.31)"]
...
| **Safe-Name Resolve API** | 내부 REST | human_name 또는 address | 매칭된 주소/이름 + 사기 DB 교차 결과 + **등록 상태·만료일 (v0.31)** | 응답 <= 500ms, 매칭 정확도 100% |
| **Safe-Name Register API** | 내부 REST | human_name, wallet_address, chain, 소유자 서명 | name_id, registered_at, expires_at, **annual_fee (v0.31)** | 유저당 5 req/day, Timeout: 5,000ms **(v0.31 변경: 오프체인 등록으로 단축)** |
...
| **KFTC OPEN API Gateway (v0.31 변경)** | 내부 REST | safe_name, account_number(선택), query_type (account_holder/corp_name) | **예금주명 또는 법인명 + 일치 여부 + Safe-Name 매핑 정보** | 이용기관 전용, Timeout: 1,000ms |
| **ISO 20022 Gateway API** | 내부 REST | safe_name, amount, msg_type (pacs.008/pacs.002) | ISO 20022 표준 메시지 (XML/JSON) | 인가 기관 전용, Timeout: 1,000ms |
...
sequenceDiagram
    ...
    alt 이름 등록됨 (status: active)
        OffChainDB-->>App: 매칭 주소 (0x…1234) + 만료일 [<= 100ms]
...
| ID | 요구사항 | Priority | Source | Acceptance Criteria |
|---|---|---|---|---|
| REQ-FUNC-014 | **(v0.31 변경)** 유저가 자신의 지갑 주소에 Human-Readable Name(예: "kim.safe")을 입력하고 등록을 완료하면, 해당 이름이 **오프체인 레지스트리 DB에 즉시 기록**되고 이후 이름으로 송금 수신이 가능해야 한다. **온체인 앵커링(Merkle Root)은 일 1회 배치로 수행한다.** | Must | Story 4, AC1 | **Given** 유저가 이름을 등록하려는 상태 **When** 이름 입력 후 등록 완료 **Then** 오프체인 DB 기록 + 이름 기반 수신 가능. 등록 완료 <= 3초(오프체인), 등록 실패율 < 0.5%, 온체인 앵커링 <= 24시간 이내 |
| REQ-FUNC-015 | 송금자가 수신자의 이름(예: "lee.safe")을 입력하고 "송금" 버튼을 탭하면, 시스템은 이름을 주소로 리졸브하고 매칭된 주소와 함께 확인 화면을 표시한 뒤 최종 승인을 거쳐 트랜잭션을 실행해야 한다. | Must | Story 4, AC2 | **Given** 송금자가 수신자 이름 입력 **When** 송금 버튼 탭 **Then** 이름→주소 리졸브 + 확인 화면 + 최종 승인 후 TX 실행. 리졸브 <= 500ms, 매칭 정확도 100% |
...
#### F4-C. 금융결제원 OPEN API 기반 예금주·법인명 조회 (v0.31 변경)

| ID | 요구사항 | Priority | Source | Acceptance Criteria |
|---|---|---|---|---|
| **REQ-FUNC-039** | **(v0.31 변경)** Safe-Name에 등록된 소유자 정보와 **금융결제원 OPEN API(오픈뱅킹)의 예금주 조회 API를 연동**하여, 이용기관(C3)이 Safe-Name으로 예금주명(사람 이름) 또는 법인명을 조회할 수 있는 게이트웨이 엔드포인트(`/api/v1/gateway/kftc`)를 제공해야 한다. Safe-Name → 주소 리졸브 후, 매핑된 계좌 정보로 금융결제원 OPEN API를 호출하여 예금주명/법인명을 반환한다. **(v0.31) MVP 시뮬레이션 시 Stub 응답 반환** | Must | CR-REVIEW-5 | **Given** TradFi 클라이언트가 KFTC OPEN API 게이트웨이 호출 **When** Safe-Name + 조회 유형(예금주/법인) 입력 **Then** 금융결제원 OPEN API 호출 → 예금주명 또는 법인명 반환. 응답 <= 1,000ms (p95). Simulation Mode 시 Stub 예금주명("홍길동") 반환 |
| **REQ-FUNC-040** | Safe-Name 송금 요청·응답 메시지가 ISO 20022 표준 메시지 구조(pacs.008 송금 지시, pacs.002 처리 상태)를 지원해야 한다. 게이트웨이 엔드포인트(`/api/v1/gateway/iso20022`)를 통해 XML/JSON 형식의 ISO 20022 메시지를 생성·반환한다. | Must | CR-3(다) | **Given** TradFi 클라이언트가 ISO 20022 게이트웨이 호출 **When** Safe-Name + 금액 + 전문 형식(iso20022) 입력 **Then** pacs.008/pacs.002 표준 메시지 생성·반환. ISO 20022 스키마 검증 통과율 100%, 응답 <= 1,000ms (p95) |
...
| ID | 요구사항 | Priority | Source | Acceptance Criteria |
|---|---|---|---|---|
| REQ-FUNC-024 | Agent는 외부 사기 정보 소스로부터 수집을 수행하여 표준 포맷으로 정규화하여 통합 DB에 적재해야 한다. 정책 변경·접근 차단된 소스는 자동 비활성화하고 대체 소스로 전환한다. **(v0.31) Simulation Mode 시 Stub 데이터 기반 수집 시뮬레이션** | Should | Story 6, AC1 | **Given** 외부 소스가 연동된 상태 **When** Agent가 수집 수행 **Then** 표준 포맷 정규화 후 통합 DB 적재. 소스별 수집 주기 <= 15분 |
...
#### F9. 운영기관 Admin Console (v0.31 신규)

| ID | 요구사항 | Priority | Source | Acceptance Criteria |
|---|---|---|---|---|
| **REQ-FUNC-048** | **(v0.31 신규)** 운영기관(O1)용 Admin Console은 다음 기능을 통합 제공해야 한다: (가) 시스템 전체 모니터링 대시보드(TPS, 가용성, 에러율), (나) Simulation/Production 모드 전환 관리, (다) NRM Adapter Registry 관리, (라) 알림 채널 어댑터 관리, (마) 이용기관·고객 계정 관리, (바) Safe-Name 비용 모델 설정(프리미엄 이름 분류 기준 등), (사) 비용 추적(외부 API 호출 비용, 인프라 비용). | Must | CR-REVIEW-4 | **Given** 운영자(O1)가 Admin Console 접속 **When** MFA 인증 완료 **Then** 전체 기능 접근 가능. 대시보드 로드 <= 3초, 모드 전환 <= 1분 |
...
| **REQ-FUNC-050** | **(v0.31 신규)** Simulation Mode에서 사용하는 시드(Seed) 데이터는 다음을 포함해야 한다: (가) 사기 주소 1,000건(위험 등급 분포: critical 10%, high 30%, medium 40%, low 20%), (나) 정상 주소 10,000건, (다) Safe-Name 등록 100건(일반 80건, 프리미엄 20건), (라) WARRANTY_POLICY 50건(active 40, expired 5, claimed 5), (마) Hotline Ticket 20건(다양한 상태). 시드 데이터는 Admin Console에서 초기화·재생성이 가능하다. | Must | CR-REVIEW-6 | **Given** 시스템이 Simulation Mode로 시작 **When** 시드 데이터 로드 **Then** 전 기능이 시드 데이터 기반으로 동작. 시드 로드 <= 30초, 전 기능 동작 검증 테스트 통과율 100% |
...
| **REQ-NF-038** | 금융결제원 OPEN API·ISO 20022 게이트웨이 응답 시간 | p95 <= 1,000ms | Datadog APM: `api.gateway.kftc.latency_p95` | CR-REVIEW-5 |
...
| **Story 4-C (KFTC OPEN API Gateway, v0.31)** | **REQ-FUNC-039** | **TC-FUNC-039** | **Must** |
| **Story 4-C (ISO 20022 Gateway)** | **REQ-FUNC-040** | **TC-FUNC-040** | **Must** |
...
| **A12** | **`/api/v1/gateway/kftc`** | **POST** | **금융결제원 OPEN API 게이트웨이 — 예금주·법인명 조회 (v0.31 변경)** | **API Key (이용기관)** | **기관당 500 req/min** | **1,000ms** |
| A13 | `/api/v1/gateway/iso20022` | POST | ISO 20022 메시지 게이트웨이 | API Key (인가 기관) | 기관당 500 req/min | 1,000ms |
...
#### KFTC_OPENAPI_LOG (v0.31 신규)

| 필드 | 타입 | 제약 | 설명 |
|---|---|---|---|
| **log_id** | **string** | **PK** | **로그 고유 ID** |
| **requesting_org_id** | **string** | **NOT NULL** | **요청 이용기관 ID** |
| **query_type** | **string** | **NOT NULL** | **조회 유형 (account_holder, corp_name)** |
| **safe_name** | **string** | **NOT NULL** | **조회 대상 Safe-Name** |
| **kftc_response_code** | **string** | **NOT NULL** | **금융결제원 응답 코드** |
| **match_result** | **boolean** | **NULLABLE** | **예금주/법인명 일치 여부** |
| **is_simulation** | **boolean** | **NOT NULL, DEFAULT false** | **시뮬레이션 모드 여부** |
| **requested_at** | **datetime** | **NOT NULL** | **요청 일시** |
| **response_latency_ms** | **int** | **NOT NULL** | **응답 지연 (ms)** |
...
#### 6.3.5 금융결제원 OPEN API 게이트웨이 플로우 (v0.31 변경)

```mermaid
sequenceDiagram
    participant TradFi as TradFi 시스템 (C3)
    participant GW as Financial Gateway Service
    participant SafeName as Safe-Name Service (Off-Chain)
    participant SimCtrl as Simulation Controller (v0.31)
    participant KFTC as 금융결제원 OPEN API
    participant StubKFTC as Stub KFTC API (Simulation)

    TradFi->>GW: POST /gateway/kftc (safe_name, query_type: account_holder)
    GW->>SafeName: Safe-Name 리졸브 (예: "kim.safe")
    SafeName-->>GW: 주소 (0x…) + 소유자 정보 + 매핑 계좌

    GW->>SimCtrl: 현재 모드 확인
    alt Simulation Mode (MVP)
        SimCtrl-->>GW: mode = simulation
        GW->>StubKFTC: Stub 예금주 조회 호출
        StubKFTC-->>GW: Stub 응답 (예금주명: "김철수", 일치: true) [<= 100ms]
    else Production Mode
        SimCtrl-->>GW: mode = production
        GW->>KFTC: OPEN API 예금주 조회 (계좌번호, 은행코드)
        KFTC-->>GW: 예금주명 + 일치 여부 [<= 800ms]
    end

    GW-->>TradFi: 예금주명/법인명 + 일치 여부 + Safe-Name 매핑 정보 [<= 1,000ms]
```
...
sequenceDiagram
    ...
    Checklist->>Checklist: ④ 금융결제원 OPEN API 인증 확인
