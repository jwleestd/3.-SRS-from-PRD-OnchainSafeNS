...
- **v0.6 보완: 품질 리뷰 14건 반영 — (1) Phase-0 REQ 19건 GWT AC 전량 반영 (2) FUNC-053/057/059 번들 해체+개별 GWT (3) FUNC-021~023, 067~069 개별 분리 (4) FUNC-002~005 AC 보충 (5) Phase-0 NFR 모니터링·위반 조치 컬럼 추가 (6) Phase-1~3 NFR 측정 방법·위반 조치 컬럼 추가 (7) 추가 NFR 번들 해체 (8) Pain ID→REQ 역추적 Matrix 추가 (9) Validation→REQ 매핑 추가 (10) Use Case Diagram 추가 (11) ERD(Mermaid) 추가 (12) Class Diagram 추가**
...
**v0.6 핵심 변경:** (1) Phase-0 REQ 19건에 PRD의 Given-When-Then AC를 전량 반영 (2) 핵심 다이어그램 3종(Use Case, ERD, Class) 추가 (3) NFR 전 항목에 모니터링·위반 조치 명세 추가 (4) 번들 요구사항 해체 (5) Pain ID 역추적 매트릭스 추가
...
### 3.5 Phase-0 Class Diagram (v0.6 신규)
...
classDiagram
    ...
    FraudService --> PrismaClient : uses
    FraudService --> EmailService : sends notifications
    SafeNameService --> PrismaClient : uses
    SafeNameService --> FraudService : crossCheck
